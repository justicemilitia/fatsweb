...
import { SERVICE_URL, GET_DASHBOARD_FIXED_ASSETS_INFO, GET_HEADERS, GET_DASHBOARD_TRANSACTIONS_INFO, GET_DASHBOARD_PERSONALS_INFO, GET_DASHBOARD_FIXED_ASSETS_COUNTS } from 'src/app/declarations/service-values';
...
export class DashboardService {
  ...
  GetDashboardFixedAssetsCount(groupType:number,success, failed) {
    this.httpClient.get(SERVICE_URL + GET_DASHBOARD_FIXED_ASSETS_COUNTS + "?groupType=" + groupType, {
      headers: GET_HEADERS(this.aService.getToken())
    }).subscribe((result: Response) => {
      if (result.ResultStatus == true) {
        success(result.ResultObject);
